...
void WriteHeader()
{
	cout << "Celeste Fischer\n"
		...
		<< "These results will be saved in text file and written to the console.\n";
}
...
//open input file and read data; takes in reference to vector and file name
bool OpenAndReadFile(string fileName, vector <encounters> &myVector)
{
	...
	{
		cout << "\n" << fileName << " opened successfully. Importing data from file.\n";
	}
	...
	{
		getline(inputFile, line, '\n'); //parses line out of document (gets next line) 
		...
		//inner while loop parses each line by comma and add data to structure
		while (getline(templine, data, ',')) //iterates through columns
		{
			...
			else //populate encounter struct with each piece of data
			{
		...
		//fill vector right after inner while loop 
		encountersKey++;
...
bool WriteFileOut(vector <encounters> &myVector, monthlyData data, string *writeFile, string *summaryString)
{
	...
	if (outputFile.is_open()) 
	{
		...
		ss << setprecision(2);
		ss << "\nCeleste Fischer\n"
			<< "The data analysis has been written to file: "<< *writeFile << "\n"
